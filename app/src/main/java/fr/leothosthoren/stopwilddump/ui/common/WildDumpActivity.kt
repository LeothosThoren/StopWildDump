...
class WildDumpActivity : AppCompatActivity() {
    ...
    private fun setUpBottomNavMenu() {
        navHomeView.setupWithNavController(navController)
        navHomeView.setOnNavigationItemReselectedListener { }
        navController.addOnDestinationChangedListener { nav, destination, _ ->
            when (destination.id) {
                R.id.destination_global_map -> navHomeView.visibility = View.VISIBLE
                R.id.destination_wild_dump_list -> navHomeView.visibility = View.VISIBLE
                else -> navHomeView.visibility = View.GONE
            }
        }
    }
